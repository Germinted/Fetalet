"""Neural network models for NIFEA Fetal Heart Rate Analysis"""
...
class CandidateGenerator(nn.Module):
    ...
    shapelets from multi-channel ECG data.
    """
...
class DualBranchModel(nn.Module):
    ...
    def __init__(self,
        ...
        # Note: For NIFEA, we adjust the input dimension calculation
        self.cnn_branch = self._build_cnn_branch(in_channels, window_size)

        # Combined classification layer
        # Input dimension: CNN features + num_prototypes
        self.fc_input_dim = 128 + num_prototypes
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Reshape for easier computation: flatten L and C dimensions
        # Shape: (batch_size, num_prototypes, num_candidates, shapelet_length * in_channels)
        diff_flat = diff.view(diff.size(0), diff.size(1), diff.size(2), -1)

        # Compute L2 norm over the flattened L*C dimension
        # Shape: (batch_size, num_prototypes, num_candidates)
        distances = torch.norm(diff_flat, dim=3)

        # Minimize over candidates to get the best match for each prototype
        # Shape: (batch_size, num_prototypes)
        min_distances, _ = torch.min(distances, dim=2)

        # CNN branch features
        cnn_features = self.cnn_branch(x)  # (batch_size, 128)
